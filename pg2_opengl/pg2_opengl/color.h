...
public:
	...
	Color(const std::array<T, N-1>& data, T last)
	{
		for(int i = 0; i < N -1; i++)
			this->data[i] = data[i];
		this->data[N - 1] = last;
	...
	Color<N, T>& operator=(const Color<N, T> lhs)
	{
		for (int i = 0; i < N; ++i)
		{
			data[i] = lhs.data[i];
	...
	Color<N, float> toLinear()
	{
		Color<N, float> linear;

		for (int i = 0; i < channels; ++i)
		{
			linear.data[i] = Color<N, float>::c_linear(data[i]);
		}

		return linear;
	}

	Color<N, float> toSRGB()
	{
		Color<N, float> srgb;

		for (int i = 0; i < channels; ++i)
		{
			srgb.data[i] = Color<N, float>::c_srgb(data[i]);
		}
