...
float ComputeShadows(float NdL)
{
    float bias = max(ShadowBias*(1.0 - NdL), ShadowBias);  
    ...
    for ( int y = -r; y <= r; ++y ) 
    {
        for ( int x = -r; x <= r; ++x ) 
        {
            ...
            depth = depth * 2.0 - 1.0;
            shadow += (depth + bias >= position_lcs.z) ? 1.0 : ShadowAmmount;
        }
    }
...
void main( void )
{
    ...
    // Compute shadows
    float shadow = ComputeShadows(NdL);
